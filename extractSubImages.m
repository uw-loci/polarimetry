...
Param.subImageWidth = 512;
Param.subImageHeight = 512;
...
Param.xRem = rem(Param.baseImgInfo.Width,Param.subImageWidth);
Param.yRem = rem(Param.baseImgInfo.Height,Param.subImageHeight);
...
Param.xRoiNum = fix((Param.subImageWidth-2*Param.ctBuffer)/Param.roiWidth);
Param.xRoiRem = rem(Param.subImageWidth-2*Param.ctBuffer,Param.roiWidth);
...
for y = 1:Param.yImgNum
    for x = 1:Param.xImgNum
        ...
        xLow = Param.xOffset+imgWidth*(x-1);
        yLow = Param.yOffset+imgHeight*(y-1);
        xHigh = xLow + imgWidth-1+2*Param.ctBuffer;
        yHigh = yLow + imgHeight-1+2*Param.ctBuffer;
        ...
        if sum(sum(subImg>Param.intensityThresh)) > Param.pixelNumThresh %A little more than 1% of pixels for 512x512
            ...
            Param = generateROIs(imgName, Param);
        end
        imgNum = imgNum+1;
    end
end
...
for y = 1:Param.yRoiNum
    for x = 1:Param.xRoiNum
        ...
        yLow = Param.yRoiOffset+Param.roiHeightt*(y-1);
        xHigh = xLow + Param.roiWidth-1;
        yHigh = yLow + Param.roiHeightt-1;
        
        separate_rois.(roiName) = tempROI;
        separate_rois.(roiName).roi = [yLow,xLow,Param.roiHeightt,Param.roiWidth];
        separate_rois.(roiName).enclosing_rect = [yLow, xLow,...
            yHigh,xHigh];
        separate_rois.(roiName).xm = xLow+Param.roiWidth/2;
        separate_rois.(roiName).ym = yLow+Param.roiHeightt/2;
